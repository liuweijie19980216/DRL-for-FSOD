...
def parse_args():
    ...
    parser.add_argument('--dataset', dest='dataset',
                        help='training dataset:coco2017,coco,pascal_voc_0712',
    ...
    parser.add_argument('--log_dir', dest='log_dir',
                        ...
                        type=str)
    parser.add_argument('--temperture', dest='temperture', default=52,
                        help='softmax_temp', type=int)
...
if __name__ == '__main__':
    ...
    cfg.temperture = args.temperture
    ...
    if args.net == 'metarcnn':
        ...
        #torch.nn.utils.clip_grad_norm(fasterRCNN.parameters(), 20)
    ...
    if args.resume:
        ...
        if args.phase == 2:
            ...
            fasterRCNN.load_state_dict(new_state_dict, strict=False)
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        ...
        for step in range(iters_per_epoch):
            ...
            rois_label, cls_prob, bbox_pred, meta_loss, group_loss = fasterRCNN(im_data_list, im_info_list,
                                                                                gt_boxes_list,
                                                                                num_boxes_list)

            if args.meta_train:
                # loss = rpn_loss_cls.mean() + rpn_loss_box.mean() + \
                #        sum(RCNN_loss_cls) / len(RCNN_loss_cls) + sum(RCNN_loss_bbox) / len(RCNN_loss_bbox) + \
                #        meta_loss / len(metaclass) + sum(group_loss) / len(group_loss)
                loss = rpn_loss_cls.mean() + rpn_loss_box.mean() + sum(RCNN_loss_cls) / args.batch_size + sum(
                    RCNN_loss_bbox) / args.batch_size + meta_loss / len(metaclass) + sum(group_loss) / args.batch_size
            else:
                loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
            ...
            optimizer.zero_grad()
            loss.backward()
            #clip_parm = [fasterRCNN.gtg.gcn.conv1.w, fasterRCNN.gtg.gcn.conv_last.w]

            # torch.nn.utils.clip_grad_value_(fasterRCNN.gtg.gcn.conv1.w, clip_value=20)
            # torch.nn.utils.clip_grad_value_(fasterRCNN.gtg.gcn.conv_last.w, clip_value=20)
            optimizer.step()
            ...
            if step % args.disp_interval == 0:
                ...
                if not args.meta_train:
                    ...
                    loss_rcnn_group = sum(group_loss) / args.batch_size
                ...
                if args.meta_train:
                    print(
                        "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f, meta_loss %.4f, loss_rcnn_group %.4f" \
                        % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, loss_metarcnn, loss_rcnn_group))
                else:
    ...
    if args.meta_train:
        with torch.no_grad():
            ...
            class_prototypes = collections.defaultdict(list)
            ...
            for i in range(shots):
                ...
                attentions, prototypes = fasterRCNN(im_data_list, im_info_list, gt_boxes_list, num_boxes_list,
                                                    average_shot=True)
                for idx, cls in enumerate(prncls):
                    class_attentions[int(cls)].append(attentions[idx])
                    class_prototypes[int(cls)].append(prototypes[idx])

        # calculate mean class data of every class
        mean_class_attentions = {k: sum(v) / len(v) for k, v in class_attentions.items()}
        mean_class_prototypes = {k: sum(v) / len(v) for k, v in class_prototypes.items()}
